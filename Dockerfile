...
RUN useradd -d $HOME -u 1000 -m -s /bin/bash jenkins && \
	...
	cd /var && wget -q http://download.java.net/glassfish/4.1/release/glassfish-4.1.zip && \
	unzip glassfish-4.1.zip && rm glassfish-4.1.zip && \
	cd $GLASSFISH_HOME/glassfish/domains/domain1/autodeploy && \
	wget -q http://mirrors.jenkins-ci.org/war/latest/jenkins.war && \
	chown jenkins:jenkins -R $JENKINS_HOME $GLASSFISH_HOME
